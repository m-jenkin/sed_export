...
pacman::p_load(readr, tidyr, dplyr, stringr, forcats, purrr, ggplot2, fs,
...
GS1 |> # check the hydrograph separation (done manually)
  group_by(year) |>
  select(year, JD, Qw) |>
  mutate(hydro = cut(JD, breaks = unique(sep$JD)), # splitting
...
meteo_summaries <- meteo_daily |>
  ...
  full_join(GS1_daily |>
              ...
              summarise(mean_mean_temp = mean(mean_temp, na.rm = T),
...
export_by_SLA <- export_by_SLA_func(
  data = {GS1_daily |> rename('SLA' = SLA_min, 'SCAR' = cover)},
...
p75 <- GS1 |>
  group_by(year) |>
  mutate(p75 = quantile(Qw, 0.75, na.rm = T),
         total_Qss = sum(Qss, na.rm = T),
         total_Qb = sum(Qb, na.rm = T))
p75a <- p75 |>
  group_by(year) |>
  summarise(sum_Qss = sum(Qss, na.rm = T),
            sum_Qb = sum(Qb, na.rm = T))
p75b <- p75 |>
  group_by(year) |>
  filter(Qw < p75) |>
  summarise(sum_Qss_below_p75 = sum(Qss, na.rm = T),
            sum_Qb_below_p75 = sum(Qb, na.rm = T))
proportions <- p75a |>
  left_join(p75b, by = "year") |>
  mutate(proportion_Qss = sum_Qss_below_p75 / sum_Qss,
...
cs_GS1 <- GS1_daily |>
  ...
  select(1:14) |>
  pivot_longer(cols = contains('Q'), names_to = 'var')

ggplot() +
  geom_line(data = cs_GS1 |> filter(str_detect(var, 'Qss')), aes(day, value, col = 'Qss', group = var)) +
  geom_line(data = cs_GS1 |> filter(str_detect(var, 'Qb')), aes(day, value, col = 'Qb', group = var)) +
  geom_line(data = cs_GS1 |> filter(str_detect(var, 'Qw')), aes(day, value * 10^3, col = 'Qw', group = var)) +
  facet_wrap(~year, scales = 'free_y') +
  ggtitle('minimal code plot to show cumulative data with uncertainty')
...
shreve <- shreve_func(glacier_outline,
                      ...
                      bed = './data/bed.tif',
                      agg = 5, # downsampling
                      dir = './data/temp/') |> suppressWarnings()
