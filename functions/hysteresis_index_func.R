...
hysteresis_func <- function(GS1, hydro_sep) {
  ...
  for(y in c(2020, 2021, 2022)){
    ...
    data <- GS1 |>
      ...
      mutate(chunk = cut(JD, breaks = sep$JD),
    ...
    for(d in unique(data$chunk)){
      ...
      suppressWarnings({
        SSC_cs <- data |> 
          filter(chunk == d) |> 
          mutate(Qw = rescale(Qw |> caTools::runmean(k = k)),
      ...
      t_Qw_max <- tibble(year = t_Qw_max_SSC$year, 
                         JD = t_Qw_max_SSC$JD, 
