...
export_by_SLA_func <- function(data, dem, glacier_outline, band_width, hysteresis) {
  ...
  elevation_bands <-
    ...
    mutate(
      proportion = as.numeric(total_area) / as.numeric(glacier_area) * 100, # proportion of glacier covered by band
      band_numeric = as.numeric(band),
      cs = cumsum(proportion)
  ...
  SLA_interpolated <- data |> # linear interpolation of SLA to 1 day resolution
    ...
    mutate(
      SLA_band =
        cut(SLA,
          breaks = seq(
            from = floor(min(SLA + 2, na.rm = TRUE) / band_width) * band_width, # assign the interpolated SLA to a band_widthm band
            to = ceiling(max(SLA + 2, na.rm = TRUE) / band_width) * band_width,
            by = band_width
          ),
          include.lowest = TRUE,
          labels = paste(seq(
            from = floor(min(SLA + 2, na.rm = TRUE) / band_width) * band_width,
  ...
  export_by_band <- SLA_interpolated |>
    ...
    mutate(
      variable = fct_relevel(variable, "cs_Qw", "cs_Qss", "cs_Qb"),
      year = as.factor(year)
    ) |>
    arrange(SLA_band) |>
